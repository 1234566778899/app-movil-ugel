...
const MonitorScreen = ({ navigation }) => {
    const w = 60;
    const { user, currentScreen, setCurrentScreen, teacherCurrent, setTeacherCurrent, startAt, setStartAt, setCurrentCourse, setDesempenios, currentCourse, currentDesempenio, desempenios, setCurrentDesempenio, education, setEducation, visit, setVisit, getQuantity } = useContext(SesionContext);
    const heightAnim = useRef(new Animated.Value(currentDesempenio == 0 ? 350 : 140)).current;
        ...
        navigation.setOptions({
            headerRight: () => (
                <Text style={{ marginRight: 15, fontSize: 16, fontWeight: 'bold', color: 'black' }}>
                    Promedio: {calcularPromedioPuntos(desempenios)}
                </Text>
            ),
        });
    }, [navigation]);
    const panResponder = useRef(
        PanResponder.create({
            ...
            onPanResponderMove: (_, gestureState) => {
                const newHeight = 350 - gestureState.dy;
                if (newHeight >= 140 && newHeight <= 350) {
    ...
    const saveMonitor = async () => {
        ...
        if (!education) return alert('Debe elegir un nivel educativo');
                    ...
                    <View style={{ flexDirection: 'row', gap: 10, marginTop: 10 }}>
                        <TouchableOpacity
                            ...
                            <Text
                                numberOfLines={2}
                                ellipsizeMode="tail"
                                style={{ fontSize: 16, fontWeight: 'bold' }}>{currentCourse || '-'}</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
